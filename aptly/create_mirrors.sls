...
add_{{ mirrorloop }}_{{keyid}}_gpg_key:
  cmd.run:
    - name: gpgv1 --no-default-keyring --keyring {{ keyring }} --keyserver {{ opts['keyserver']|default('keys.gnupg.net') }} --recv-keys {{keyid}}
    - runas: aptly
    - unless: gpgv1 --list-keys --keyring {{ keyring }}  | grep {{keyid}}
{% endfor %}
...
add_{{ mirror }}_gpg_key:
  cmd.run:
    - name: gpgv1 --no-default-keyring --keyring {{ keyring }} --keyserver {{ opts['keyserver']|default('keys.gnupg.net') }} --recv-keys {{ opts['keyid'] }}
    - runas: aptly
    - unless: gpgv1 --list-keys --keyring {{ keyring }}  | grep {{ opts['keyid'] }}
  {% elif opts['key_url'] is defined %}
      - cmd: add_{{ mirror }}_gpg_key

add_{{ mirror }}_gpg_key:
  cmd.run:
    - name: gpgv1 --no-default-keyring --keyring {{ keyring }} --fetch-keys {{ opts['key_url'] }}
    - runas: aptly
    - unless: gpgv1 --list-keys --keyring {{ keyring }}  | grep {{keyid}}
  {% endif %}
{% endfor %}
